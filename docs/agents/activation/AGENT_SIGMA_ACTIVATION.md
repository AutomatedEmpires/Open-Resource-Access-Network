...
- **SSOT alignment**: when you change an API contract (request shape, response shape, status
  codes, auth requirements), update `docs/platform/INTEGRATIONS.md` and the relevant route README.
...
- **Status output**: at the end of your session, write a complete structured status report to
  `docs/agents/status/STATUS_SIGMA.md` using the format defined at the bottom of this file.
...
docs/governance/ROLES_PERMISSIONS.md         # You update this file
docs/platform/INTEGRATIONS.md              # You update API contracts here
```
...
5. `docs/governance/ROLES_PERMISSIONS.md` — ORAN role hierarchy and what each role may access
6. `docs/CHAT_ARCHITECTURE.md` — chat pipeline architecture (crisis gate ordering is defined here)
7. `docs/platform/INTEGRATIONS.md` — existing API contract documentation
...
- Verify the role ordering (lower→higher privilege) is correctly defined and used by
  `isRoleAtLeast`.
- Update `docs/governance/ROLES_PERMISSIONS.md` to reflect the exact implemented role set.
...
- Update `docs/platform/INTEGRATIONS.md` to reflect the current, accurate API surface.
...
- [ ] `docs/agents/status/STATUS_SIGMA.md` written with the full structured report.
- [ ] `npx tsc --noEmit` passes with zero errors across all owned files.
- [ ] `npm run lint` passes with zero errors across all owned files.

---

## 7. Status Report Format (`docs/agents/status/STATUS_SIGMA.md`)
