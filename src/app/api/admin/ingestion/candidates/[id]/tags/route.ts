...
export async function GET(
  ...
  const ip = getIp(req);
  const rl = await checkRateLimitShared(`admin:ingestion:candidates:tags:read:${ip}`, { maxRequests: ORAN_ADMIN_READ_RATE_LIMIT_MAX_REQUESTS, windowMs: RATE_LIMIT_WINDOW_MS });
...
export async function PUT(
  ...
  const ip = getIp(req);
  const rl = await checkRateLimitShared(`admin:ingestion:candidates:tags:write:${ip}`, { maxRequests: ORAN_ADMIN_WRITE_RATE_LIMIT_MAX_REQUESTS, windowMs: RATE_LIMIT_WINDOW_MS });
  if (rl.backendUnavailable) {
    return NextResponse.json(
      { error: 'Rate limit service unavailable. Please try again later.' },
      { status: 503, headers: { 'Retry-After': String(rl.retryAfterSeconds) } },
