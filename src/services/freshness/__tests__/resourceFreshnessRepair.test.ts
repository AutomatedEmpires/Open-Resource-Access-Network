...
describe('repairResourceFreshnessPacketInTransaction', () => {
  ...
  it('repairs a missing packet while preserving unrelated payload and removing stale review data', async () => {
    const originalPayload = {
      unrelated: { keep: true },
      correlationId: 'correlation-1',
      resourceFreshnessReview: {
        outcome: 'confirmed_current',
        reviewerSummary: 'This stale review must not survive packet reconstruction.',
      },
    };
    ...
    expect(JSON.parse(String(auditParams[1]))).toEqual({
      ...
      removedStaleReview: true,
    });
    expect(String(auditParams[1])).not.toContain('serviceUpdatedAt');
    expect(String(auditParams[1])).not.toContain('lastSourceRefreshAt');
  });

  it('repairs an invalid packet and records that state without retaining it', async () => {
    const client = clientFor(target({
      payload: {
        keep: 'yes',
        resourceFreshness: { schemaVersion: 999, raw: 'invalid' },
      },
    }));
    ...
    expect(JSON.parse(String(auditCall?.[1]?.[1]))).toMatchObject({
      ...
      removedStaleReview: false,
    });
  });

  it('does not overwrite a packet that already passes the strict schema', async () => {
  ...
  it.each([
    ...
    ['signal-column mismatch', { signal_observed_at: '2026-01-02T00:00:00.000Z' }],
