...
CREATE TABLE IF NOT EXISTS oran_internal.account_erasure_release_gate (
  singleton boolean PRIMARY KEY DEFAULT true CHECK (singleton),
  indexes_ready boolean NOT NULL DEFAULT false,
  updated_at timestamptz NOT NULL DEFAULT now()
);
INSERT INTO oran_internal.account_erasure_release_gate (singleton, indexes_ready)
VALUES (true, false)
...
-- Existing-table indexes are installed online by the 0072 operator phase.
...
ALTER TABLE oran_internal.account_erasure_release_gate ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON TABLE oran_internal.account_erasure_release_gate FROM PUBLIC;
...
RETURNS TABLE (request_id uuid, request_status text, lease_acquired boolean)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $function$
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM oran_internal.account_erasure_release_gate gate
    WHERE gate.singleton AND gate.indexes_ready
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'account erasure capability is not ready';
  END IF;
  ...
  FOR UPDATE;
  IF FOUND THEN
    IF v_request.status IN ('pending', 'processing')
       AND (v_request.lease_expires_at IS NULL
            OR v_request.lease_expires_at <= pg_catalog.now())
       AND v_request.next_attempt_at <= pg_catalog.now() THEN
    ...
    END IF;
    RETURN QUERY SELECT v_request.id, v_request.status, v_lease_acquired;
    RETURN;
  END IF;

  INSERT INTO oran_internal.account_erasure_requests (
    user_id, clerk_user_id, user_digest, clerk_user_digest,
    text_tombstone, uuid_tombstone, profile_id, status, lease_expires_at
  ) VALUES (
    p_user_id, p_clerk_user_id, v_user_digest, v_clerk_digest,
    p_text_tombstone, p_uuid_tombstone, v_profile_id, 'processing',
...
CREATE OR REPLACE FUNCTION oran_internal.release_account_erasure_lease(
  p_request_id uuid
)
RETURNS void
LANGUAGE sql
...
BEGIN
  ...
  v_identity_patterns := ARRAY[
    oran_internal.regex_quote(v_request.user_id)
  ];
  IF v_request.clerk_user_id IS DISTINCT FROM v_request.user_id THEN
    v_identity_patterns := pg_catalog.array_append(
      v_identity_patterns,
  ...
  CASE v_step.step_name
    ...
    WHEN 'form_instances' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE instance.recipient_user_id = v_request.user_id
           OR instance.form_data::text ~ ANY(v_identity_patterns)
           OR instance.attachment_manifest::text ~ ANY(v_identity_patterns)
           OR instance.blob_storage_prefix ~ ANY(v_identity_patterns)
           OR EXISTS (
      ...
      IF v_high IS NOT NULL THEN
        ...
        WHERE row.id = ANY(v_page)
          AND (
            ...
            OR row.form_data::text ~ ANY(v_identity_patterns)
            OR row.attachment_manifest::text ~ ANY(v_identity_patterns)
            OR row.blob_storage_prefix ~ ANY(v_identity_patterns)
          );
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'source_records' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (raw_payload::text ~ ANY(v_identity_patterns)
               OR parsed_payload::text ~ ANY(v_identity_patterns)
               OR source_confidence_signals::text ~ ANY(v_identity_patterns)
               OR processing_error ~ ANY(v_identity_patterns)
               OR EXISTS (
                 SELECT 1 FROM public.submissions authored
      ...
      IF v_high IS NOT NULL THEN
        ...
        WITH sanitized AS MATERIALIZED (
          ...
          WHERE record.id = ANY(v_page)
            AND (
              record.raw_payload::text ~ ANY(v_identity_patterns)
              OR record.parsed_payload::text ~ ANY(v_identity_patterns)
              OR record.source_confidence_signals::text ~ ANY(v_identity_patterns)
              OR record.processing_error ~ ANY(v_identity_patterns)
              OR EXISTS (
                SELECT 1 FROM public.submissions authored
                WHERE authored.submitted_by_user_id = v_request.user_id
                  AND authored.id::text = record.raw_payload ->> 'submissionId'
    ...
    WHEN 'submissions' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (submitted_by_user_id = v_request.user_id
               ...
               OR payload::text ~ ANY(v_identity_patterns)
               OR evidence::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (
            ...
            OR row.payload::text ~ ANY(v_identity_patterns)
            OR row.evidence::text ~ ANY(v_identity_patterns)
          );
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'extracted_candidates' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (assigned_to_user_id = v_request.user_id
               ...
               OR verification_checklist::text ~ ANY(v_identity_patterns)
               OR investigation_pack::text ~ ANY(v_identity_patterns)
               OR provenance_records::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (
            ...
            OR row.verification_checklist::text ~ ANY(v_identity_patterns)
            OR row.investigation_pack::text ~ ANY(v_identity_patterns)
            OR row.provenance_records::text ~ ANY(v_identity_patterns)
          );
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'verification_queue_archive' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE assigned_to_user_id = v_request.user_id
           ...
           OR created_by_user_id = v_request.user_id
           OR updated_by_user_id = v_request.user_id
           OR notes ~ ANY(v_identity_patterns)
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.assigned_to_user_id = v_request.user_id
               ...
               OR row.updated_by_user_id = v_request.user_id
               OR row.notes ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'ownership_transfers' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (requested_by_user_id = v_request.user_id
               ...
               OR service_snapshot::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.requested_by_user_id = v_request.user_id
               ...
               OR row.service_snapshot::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'ingestion_audit_events' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (actor_id = v_request.user_id
               OR details::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.actor_id = v_request.user_id
               OR row.details::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'lifecycle_events' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (actor_id = v_request.user_id
               OR metadata::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.actor_id = v_request.user_id
               OR row.metadata::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'audit_logs' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (actor_user_id = v_request.user_id
               OR before::text ~ ANY(v_identity_patterns)
               OR after::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.actor_user_id = v_request.user_id
               OR row.before::text ~ ANY(v_identity_patterns)
               OR row.after::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'scope_audit_log' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (actor_user_id = v_request.user_id
               OR target_id ~ ANY(v_identity_patterns)
               OR before_state::text ~ ANY(v_identity_patterns)
               OR after_state::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.actor_user_id = v_request.user_id
               OR row.target_id ~ ANY(v_identity_patterns)
               OR row.before_state::text ~ ANY(v_identity_patterns)
               OR row.after_state::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'submission_transitions' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (actor_user_id = v_request.user_id
               OR gates_checked::text ~ ANY(v_identity_patterns)
               OR metadata::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        ...
        WHERE row.id = ANY(v_page)
          AND (row.actor_user_id = v_request.user_id
               OR row.gates_checked::text ~ ANY(v_identity_patterns)
               OR row.metadata::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'resource_quarantine_batches' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (created_by = v_request.user_id
               OR classifier::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.created_by = v_request.user_id
               OR row.classifier::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'hotline_authority_members' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE original_service::text ~ ANY(v_identity_patterns)
           OR applied_service::text ~ ANY(v_identity_patterns)
           OR original_organization::text ~ ANY(v_identity_patterns)
           OR applied_organization::text ~ ANY(v_identity_patterns)
           OR original_phones::text ~ ANY(v_identity_patterns)
           OR applied_phones::text ~ ANY(v_identity_patterns)
        ORDER BY batch_id DESC, service_id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; v_high_2 := v_step.highwater_uuid_2; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(batch_id ORDER BY batch_id, service_id), ARRAY[]::uuid[]),
        ...
        FROM (SELECT batch_id, service_id
              FROM oran_internal.hotline_authority_members
              WHERE (
                  original_service::text ~ ANY(v_identity_patterns)
                  OR applied_service::text ~ ANY(v_identity_patterns)
                  OR original_organization::text ~ ANY(v_identity_patterns)
                  OR applied_organization::text ~ ANY(v_identity_patterns)
                  OR original_phones::text ~ ANY(v_identity_patterns)
        ...
        FROM pg_catalog.unnest(v_page, v_page_2) page(batch_id, service_id)
        WHERE row.batch_id = page.batch_id AND row.service_id = page.service_id
          AND (row.original_service::text ~ ANY(v_identity_patterns)
               OR row.applied_service::text ~ ANY(v_identity_patterns)
               OR row.original_organization::text ~ ANY(v_identity_patterns)
               OR row.applied_organization::text ~ ANY(v_identity_patterns)
               OR row.original_phones::text ~ ANY(v_identity_patterns)
               OR row.applied_phones::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'hotline_quarantined_contacts' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE phone_snapshot::text ~ ANY(v_identity_patterns)
      ...
      IF v_high IS NOT NULL THEN
        ...
        WHERE row.batch_id = page.batch_id AND row.phone_id = page.phone_id
          AND row.phone_snapshot::text ~ ANY(v_identity_patterns);
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
    ...
    WHEN 'hotline_authority_batches' THEN
      IF NOT v_step.highwater_captured THEN
        ...
        WHERE (created_by = v_request.user_id
               OR validation_summary::text ~ ANY(v_identity_patterns))
        ORDER BY id DESC LIMIT 1;
      ELSE v_high := v_step.highwater_uuid; END IF;
      IF v_high IS NOT NULL THEN
        SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[]),
        ...
        WHERE row.id = ANY(v_page)
          AND (row.created_by = v_request.user_id
               OR row.validation_summary::text ~ ANY(v_identity_patterns));
        GET DIAGNOSTICS v_changed = ROW_COUNT;
      END IF;
...
REVOKE ALL ON FUNCTION oran_internal.claim_account_erasure_requests(integer)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION oran_internal.release_account_erasure_lease(uuid)
  FROM PUBLIC;
...
BEGIN
  ...
  LOOP
    IF pg_catalog.to_regrole(v_role) IS NOT NULL THEN
      ...
      EXECUTE pg_catalog.format(
        'REVOKE ALL ON TABLE '
        || 'oran_internal.account_erasure_release_gate FROM %I',
        v_role
      );
      EXECUTE pg_catalog.format(
  ...
  IF pg_catalog.to_regrole('oran_backend_runtime') IS NOT NULL THEN
    ...
    EXECUTE 'GRANT EXECUTE ON FUNCTION '
      ...
      || 'TO oran_backend_runtime';
    EXECUTE 'GRANT EXECUTE ON FUNCTION '
      || 'oran_internal.release_account_erasure_lease(uuid) '
      || 'TO oran_backend_runtime';
    EXECUTE 'GRANT EXECUTE ON FUNCTION '
...
COMMENT ON TABLE oran_internal.account_erasure_release_gate IS
  'Private activation marker set only by the tracked post-index gate migration.';
...
COMMENT ON FUNCTION oran_internal.queue_account_erasure(text, text, text, uuid) IS
  'Queues an idempotent request, freezes authorization, and leases first processing atomically.';
COMMENT ON FUNCTION oran_internal.release_account_erasure_lease(uuid) IS
  'Releases an incomplete worker lease so another bounded invocation can resume immediately.';
