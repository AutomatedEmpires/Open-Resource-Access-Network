...
| --- | --- | --- | --- | --- | --- | --- |
| Public, seeker, host, admin | Sign in selector | choose seeker, organization, or admin intent; authenticate in the dedicated ORAN Clerk flow | Clerk `<SignIn>` plus `/api/auth/context` | public identity entry; ORAN roles resolved server-side | creates a Clerk session, then ORAN resolves database-owned account status, role, and membership | `src/app/auth/signin/SignInPageClient.tsx`, `src/app/api/auth/context/route.ts` |
| Public | Create account | complete the dedicated ORAN Clerk sign-up flow | Clerk `<SignUp>` | public with Clerk bot/abuse controls | Clerk creates the identity; the first explicit profile save creates a `seeker` ORAN profile mapped by Clerk ID, never by email inference | `src/app/auth/signup/[[...signup]]/page.tsx`, `src/app/api/profile/route.ts` |
| Public or authenticated seeker | Submit resource suggestion | launch structured resource cards, optionally analyze a pasted source URL into the same canonical draft, save local draft, and submit for review | `POST /api/resource-submissions`, `GET` or `PUT /api/resource-submissions/[id]`, `POST /api/resource-submissions/[id]/assist` | anonymous allowed for public channel; assist also accepts resume token for public drafts | creates submission-backed `form_instances` and `submissions`, stores local resume token for anonymous drafts, and can suggest missing canonical fields, tags, and evidence notes from the cited source page without bypassing review | `src/app/(seeker)/submit-resource/SubmitResourcePageClient.tsx`, `src/components/resource-submissions/ResourceSubmissionWorkspace.tsx`, `src/app/api/resource-submissions/route.ts`, `src/app/api/resource-submissions/[id]/route.ts`, `src/app/api/resource-submissions/[id]/assist/route.ts` |
...
| Authenticated seeker | Manage sign-in security | open account menu for password, connected login, passkey, or MFA settings | Clerk account management | authenticated Clerk user | identity credentials remain in Clerk; ORAN does not receive or mutate password hashes | `src/components/ui/portal-user-menu.tsx`, `src/app/(seeker)/profile/ProfilePageClient.tsx` |
...
| Host org user | Create or edit listing in Resource Studio | organization, service, location, taxonomy, access, evidence, and review cards, plus source-link analysis into the same draft | `POST /api/resource-submissions`, `GET` or `PUT /api/resource-submissions/[id]`, `POST /api/resource-submissions/[id]/assist` | org-scoped host access | creates or updates one submission-backed listing draft, can suggest missing canonical fields from an official source page before submit, stores source assertion on submit, and keeps review status attached to the same listing packet | `src/app/(host)/resource-studio/ResourceStudioPageClient.tsx`, `src/app/(host)/services/ServicesPageClient.tsx`, `src/components/resource-submissions/ResourceSubmissionWorkspace.tsx`, `src/app/api/resource-submissions/route.ts`, `src/app/api/resource-submissions/[id]/route.ts`, `src/app/api/resource-submissions/[id]/assist/route.ts` |
...
| Community admin | Detailed verification decision | review shared submission cards, optionally rerun source-link analysis against the same draft, then approve, deny, escalate, or return with required notes rules | `GET` or `PUT /api/resource-submissions/[id]`, `POST /api/resource-submissions/[id]/assist`, `PUT /api/community/queue/[id]` | `community_admin` | renders the same submission-backed resource cards when available, lets reviewers compare source-derived suggestions against incomplete fields, writes reviewer notes, advances workflow, updates confidence scoring, and applies verified payload on approval | `src/app/(community-admin)/verify/VerifyPageClient.tsx`, `src/components/resource-submissions/ResourceSubmissionWorkspace.tsx`, `src/app/api/resource-submissions/[id]/route.ts`, `src/app/api/resource-submissions/[id]/assist/route.ts`, `src/app/api/community/queue/[id]/route.ts` |
...
- Shared auth: `src/app/auth/signin/SignInPageClient.tsx`, `src/app/auth/signup/[[...signup]]/page.tsx`, `src/app/api/auth/context/route.ts`
