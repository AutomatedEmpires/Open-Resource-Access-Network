    ...
    <ErrorBoundary>
      ...
      <div className="grid gap-4">
        {data?.results.map((row) => {
          ...
          return (
            <section key={row.id} className={`rounded-2xl border bg-white p-5 ${row.is_high_risk ? 'border-amber-300' : 'border-gray-200'}`}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-base font-semibold text-gray-900">{row.title ?? 'Community report'}</h2>
                    ...
                    {row.integrity_hold_at ? (
                      <span className="inline-flex rounded-full bg-rose-100 px-2.5 py-1 text-xs font-medium text-rose-800">integrity hold active</span>
                    ) : null}
                  </div>
                  <p className="text-sm text-gray-600">{row.organization_name ?? 'Unknown organization'} • {row.service_name ?? 'Unknown service'} • {row.reason?.replace(/_/g, ' ') ?? 'unspecified reason'}</p>
                  <p className="text-sm text-gray-500">{row.notes}</p>
                  <p className="text-xs text-gray-400">Opened {new Date(row.created_at).toLocaleString()}.</p>
                </div>
