...
- **SSOT alignment**: when you change a page layout, component contract, or i18n string
  key, update `docs/ui/UI_SURFACE_MAP.md`, `docs/ui/UI_UX_CONTRACT.md`, or `docs/solutions/I18N_WORKFLOW.md`
  as appropriate.
...
- **Status output**: at the end of your session, write a complete structured status report to
  `docs/agents/status/STATUS_OMEGA.md` using the format defined at the bottom of this file.
...
docs/ui/UI_UX_CONTRACT.md         # Read and enforce — propose changes via ADR if needed
docs/ui/UI_UX_TOKENS.md           # Read and enforce — design parameters
docs/CHAT_ARCHITECTURE.md         # Read — crisis gate ordering is defined here
docs/ui/UX_FLOWS.md               # Read — authoritative user flow definitions
docs/audit/AUDIT_REPORT.md        # Read — overall repo truth map
docs/audit/AUDIT_SEEKER_UX.md     # Read — seeker-specific audit findings
```
...
4. `docs/ui/UI_UX_CONTRACT.md` — the authoritative UI/UX non-negotiables (read every section)
5. `docs/ui/UI_UX_TOKENS.md` — concrete design parameters (breakpoints, spacing, typography)
6. `docs/ui/UX_FLOWS.md` — authoritative user flow definitions
7. `docs/audit/AUDIT_REPORT.md` — point-in-time repo audit
8. `docs/audit/AUDIT_SEEKER_UX.md` — seeker-specific audit findings
9. `docs/CHAT_ARCHITECTURE.md` — chat pipeline including crisis gate
10. `docs/solutions/I18N_WORKFLOW.md` — i18n workflow and conventions
11. `docs/ui/UI_SURFACE_MAP.md` — complete surface map of all routes and components
12. `docs/governance/PAGE_DEFINITION_OF_DONE.md` — per-page completion definition
...
- [ ] `docs/ui/UI_SURFACE_MAP.md` reflects current route structure accurately.
- [ ] `docs/ENGINEERING_LOG.md` updated for every contract-level change.
- [ ] `docs/agents/status/STATUS_OMEGA.md` written with the full structured report.
- [ ] `npx tsc --noEmit` passes with zero errors across all owned files.
- [ ] `npm run lint` passes with zero errors across all owned files.

---

## 7. Status Report Format (`docs/agents/status/STATUS_OMEGA.md`)
