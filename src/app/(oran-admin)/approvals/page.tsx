 ...
 * Lists verification_queue entries with approve/deny actions.
...
import {
  ...
  Building2, Clock, ExternalLink, Mail, Filter,
} from 'lucide-react';
...
const STATUS_STYLES: Record<VerificationStatus, { color: string; label: string }> = {
  ...
  verified:  { color: 'bg-green-100 text-green-800 ring-green-600/20',   label: 'Approved' },
  rejected:  { color: 'bg-red-100 text-red-800 ring-red-600/20',         label: 'Denied' },
...
function StatusBadge({ status }: { status: VerificationStatus }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${s.color}`}>
      {s.label}
    </span>
  );
}
...
function ApprovalsPageInner() {
  ...
  // ── Submit decision ──
  const handleDecision = useCallback(async (queueEntryId: string, decision: 'approved' | 'denied') => {
  ...
  }, [decisionNotes, page, statusFilter, fetchClaims]);
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Review and approve or deny organization ownership claims.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {!isLoading && data && data.results.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-lg border border-gray-200 bg-white p-12 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">
            {statusFilter
              ? `No claims with status "${STATUS_STYLES[statusFilter]?.label}".`
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {data.results.map((row) => {
                  ...
                  const canDecide = ['pending', 'in_review'].includes(row.status);
                  return (
