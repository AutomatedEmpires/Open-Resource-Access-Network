...
export async function GET(req: NextRequest) {
  ...
  const ip = getIp(req);
  const rl = await checkRateLimitShared(`resource-submissions:read:${ip}`, {
...
export async function POST(req: NextRequest) {
  ...
  const ip = getIp(req);
  const rl = await checkRateLimitShared(`resource-submissions:write:${ip}`, {
    windowMs: RATE_LIMIT_WINDOW_MS,
  ...
  try {
    const submittedByUserId = authCtx?.userId ?? buildAnonymousUserId();
