# Agent B — Frontend: Pages, Components, & UX Wiring
...
2. **Read the audit**: `docs/AUDIT_SEEKER_UX.md` — sections 2, 5, and 7 are your primary input.
3. **Read Agent A's contract**: `docs/WORKLOAD_AGENT_A.md` — sections A2/A3/A4 define the API shapes you code against. If Agent A's APIs are not yet live, your code must still compile and work with graceful fallback (try the API, fall back to localStorage if it returns 401 or fails).
