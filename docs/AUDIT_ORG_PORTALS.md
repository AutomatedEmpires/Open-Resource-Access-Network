...
| ID | Severity | Finding |
|----|----------|---------|
| COM-COV-1 | **High** | The `coverage_zones` table does not yet exist. The page comment says: *"Note: coverage_zones table does not exist yet (see AGENT_PROMPT_SQL.md). When it ships, this page will show zone boundary + per-zone filters."* The stat cards and activity chart currently show aggregate totals for **all** queue entries, not just the admin's zone. This means a community admin believes they are seeing their zone but actually sees the global queue. |
