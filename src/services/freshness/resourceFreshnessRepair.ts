...
function reconstructPacket(target: RepairTargetRow): ResourceFreshnessReviewPacket | null {
  const packet = resourceFreshnessReviewPacketSchema.safeParse({
    ...
    reviewRequirements: {
      ...
      scheduleCorrectionRequiredBeforeApproval: target.signal_type === 'explicit_expiry',
    },
  });
  if (!packet.success) return null;

  // The private finding is authoritative. Its denormalized evidence must still
  // describe the exact signal and block instant recorded in typed columns.
  if (
    !sameInstant(packet.data.observed.signalObservedAt, target.signal_observed_at)
    || !sameInstant(packet.data.observed.detectedAsOf, target.blocked_at)
    || packet.data.observed.freshnessThresholdDays !== target.freshness_threshold_days
  ) {
    return null;
  }
...
export async function repairResourceFreshnessPacketInTransaction(
  ...
  const currentPacket = resourceFreshnessReviewPacketSchema.safeParse(
    payloadRecord?.resourceFreshness,
  );
  if (currentPacket.success) {
  ...
  const repairedPayload: Record<string, unknown> = payloadRecord
    ? { ...payloadRecord }
    : {};
  delete repairedPayload.resourceFreshnessReview;
  ...
  const auditMetadata = {
    ...
    previousPacketState: payloadRecord
      ...
      ? 'invalid'
      : 'missing',
    removedStaleReview: Boolean(
      payloadRecord
      && Object.prototype.hasOwnProperty.call(payloadRecord, 'resourceFreshnessReview'),
    ),
  };
  await client.query(
