...
describe('canonicalServiceStore', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('getById returns first row or null', async () => {
    ...
    const store = createDrizzleCanonicalServiceStore(db as never);

    expect(await store.getById('csvc-1')).toEqual(row);
    expect(await store.getById('nope')).toBeNull();
  });

  it('listByOrganization returns services for org', async () => {
    const rows = [makeRow(), makeRow({ id: 'csvc-2' })];
  ...
  it('listByLifecycle returns matching services', async () => {
  ...
  it('listByPublication returns matching services', async () => {
  ...
  it('listByWinningSource returns services from a source system', async () => {
  ...
  it('update omits id and createdAt from set', async () => {
    ...
    expect(set).toHaveProperty('name', 'Updated Service');
  ...
  it('updateLifecycleStatus sets status', async () => {
    ...
    await store.updateLifecycleStatus('csvc-1', 'verified');
    const set = updateSets[0] as Record<string, unknown>;
  ...
  it('updatePublicationStatus sets status', async () => {
    ...
    await store.updatePublicationStatus('csvc-1', 'published');
    const set = updateSets[0] as Record<string, unknown>;
