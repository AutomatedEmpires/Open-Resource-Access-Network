...
describe('off-Azure runtime provider policy', () => {
  ...
  it.each([
    'postgresql://postgres.project:secret@aws-0-us-west-1.pooler.supabase.com:6543/postgres',
    'rediss://default:secret@oran.upstash.io:6379',
  ...
  it('binds migration DSNs to the expected Supabase project', () => {
    ...
    const direct = `postgresql://postgres:secret@db.${projectRef}.supabase.co:5432/postgres`;
    const pooler = `postgresql://postgres.${projectRef}:secret@aws-0-us-west-1.pooler.supabase.com:6543/postgres`;
    ...
    expect(isExpectedSupabaseProjectDatabaseEndpoint(
      `postgresql://postgres.${otherProjectRef}:secret@aws-0-us-west-1.pooler.supabase.com:6543/postgres`,
      projectRef,
    )).toBe(false);
    expect(isExpectedSupabaseProjectDatabaseEndpoint(
      `postgresql://postgres.${projectRef}:secret@pooler.supabase.com.evil.example:6543/postgres`,
      projectRef,
    )).toBe(false);
  });
  ...
  it('rejects a cross-portfolio Supabase DSN without disclosing either identifier', () => {
    const projectRef = 'tpatxospkuqvajusuryw';
    const otherProjectRef = 'abcdefghijklmnopqrst';
    const endpoint = `postgresql://postgres.${otherProjectRef}:do-not-print@aws-0-us-west-1.pooler.supabase.com:6543/postgres`;
