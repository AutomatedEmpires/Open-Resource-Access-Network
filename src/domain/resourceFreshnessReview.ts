...
/**
 * The FLAT packet main's freshness engine actually writes into
 * submissions.payload.resourceFreshness (buildEvidence output + findingId in
 * src/services/freshness/resourceFreshness.ts). The structured review packet
 * above is a derived view of this contract, produced on demand for review
 * surfaces via deriveResourceFreshnessReviewPacket.
 */
export const resourceFreshnessPayloadPacketSchema = z.object({
  findingId: z.string().uuid(),
  scannerVersion: z.literal(RESOURCE_FRESHNESS_REVIEW_SCHEMA_VERSION),
  signalType: resourceFreshnessSignalSchema,
  detectedAsOf: z.string().datetime({ offset: true }),
  signalObservedAt: z.string().datetime({ offset: true }),
  freshnessThresholdDays: z.number().int().positive().nullable(),
  serviceUpdatedAt: z.string().datetime({ offset: true }),
  ...
  datedScheduleCount: z.number().int().nonnegative(),
  maxValidTo: nullableDateSchema,
}).strict().superRefine((packet, ctx) => {
  if (packet.datedScheduleCount > packet.scheduleCount) {
    ctx.addIssue({
      code: 'custom',
      path: ['datedScheduleCount'],
      message: 'datedScheduleCount cannot exceed scheduleCount',
    });
  }
});

export type ResourceFreshnessPayloadPacket = z.infer<
  typeof resourceFreshnessPayloadPacketSchema
>;

/**
 * Derive the structured review-facing packet from the flat stored contract.
 * Validates the result through resourceFreshnessReviewPacketSchema so any
 * drift between the two contracts fails loudly instead of silently.
 */
export function deriveResourceFreshnessReviewPacket(
  flat: ResourceFreshnessPayloadPacket,
  ...
  return resourceFreshnessReviewPacketSchema.parse({
    ...
    findingId: flat.findingId,
    signal: flat.signalType,
    requiredAction: requiredActionForResourceFreshnessSignal(flat.signalType),
    hold: {
      actor: RESOURCE_FRESHNESS_SCANNER_ACTOR,
      reason: `resource_freshness:${flat.signalType}:${flat.findingId}`,
    },
    observed: {
      detectedAsOf: flat.detectedAsOf,
      signalObservedAt: flat.signalObservedAt,
      freshnessThresholdDays: flat.freshnessThresholdDays,
      serviceUpdatedAt: flat.serviceUpdatedAt,
      lastSourceRefreshAt: flat.lastSourceRefreshAt,
      lastCandidateVerifiedAt: flat.lastCandidateVerifiedAt,
      lastManualVerificationAt: flat.lastManualVerificationAt,
      reverifyAt: flat.reverifyAt,
      schedule: {
        totalCount: flat.scheduleCount,
        datedCount: flat.datedScheduleCount,
        maxValidTo: flat.maxValidTo,
      },
    },
    reviewRequirements: {
      evidenceRequired: true,
      scheduleCorrectionRequiredBeforeApproval: flat.signalType === 'explicit_expiry',
    },
  });
}
