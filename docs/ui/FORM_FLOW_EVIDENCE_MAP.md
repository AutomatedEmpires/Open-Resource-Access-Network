...
| Public | Register account | display name, email, password, optional phone | `POST /api/auth/register` | public with rate limit | inserts `user_profiles` credentials account with role `seeker`, trims and normalizes identity fields before uniqueness checks, rejects identity-derived or weak passwords, and routes a hidden honeypot field from the public sign-up form so low-signal bot submissions can be absorbed without creating an account | `src/app/auth/signin/SignInPageClient.tsx`, `src/app/api/auth/register/route.ts` |
