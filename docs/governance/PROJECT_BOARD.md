...
### Dispatch inputs (what the UI fields mean)

- `mode`
  - `backfill-open`: scans open issues/PRs (most recently updated first) and syncs fields
  - `single`: syncs exactly one issue/PR by number
- `limit`
  - Only used for `backfill-open`
  - Clamped to 1–200 by the workflow
- `kind`
  - Only used for `single`
  - Choose `issue` or `pr`
- `number`
  - Only used for `single`
  - The issue/PR number (example: `24`)
