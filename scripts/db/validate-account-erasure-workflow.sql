...
BEGIN
  ...
  VALUES (v_user_id, 'must be erased');

  INSERT INTO public.audit_logs (actor_user_id, action, resource_type, after)
  VALUES (
    v_user_id,
    'workflow_fixture',
    'test',
    pg_catalog.jsonb_build_object(
      'user', v_user_id,
      'provider', 'embedded/' || v_clerk_user_id || '/value',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'owned saved collection survived its deletion step';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE log.id = v_audit_id
      AND (
        log.actor_user_id IN (v_user_id, v_clerk_user_id)
        OR log.before::text ~ ANY(ARRAY[
          oran_internal.regex_quote(v_user_id),
          oran_internal.regex_quote(v_clerk_user_id),
          oran_internal.regex_quote(v_profile_id::text)
        ])
        OR log.after::text ~ ANY(ARRAY[
          oran_internal.regex_quote(v_user_id),
          oran_internal.regex_quote(v_clerk_user_id),
          oran_internal.regex_quote(v_profile_id::text)
        ])
    ...
    RAISE EXCEPTION 'preserved audit row retained a raw identity';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.audit_logs log WHERE log.id = v_audit_id
  ) THEN
    RAISE EXCEPTION 'erasure deleted the preserved governance audit row';
  END IF;
  IF EXISTS (
    SELECT 1
