...
Until the repo formalizes this (e.g., in `docs/governance/GOVERNANCE.md`), this audit uses the following precedence rules:
...
- `docs/solutions/IMPORT_PIPELINE.md#pipeline-stages`
- `docs/solutions/IMPORT_PIPELINE.md#staging-tables`
...
- `docs/platform/INTEGRATIONS.md#authentication-microsoft-entra-id`
- `docs/platform/INTEGRATIONS.md#feature-flags`
...
- `docs/ui/UI_SURFACE_MAP.md#seeker-routes-public-facing`
- `docs/ui/UI_SURFACE_MAP.md#host-routes-organization-management`
...
- `docs/governance/GOVERNANCE.md#safety-critical-norms`
...
- `docs/governance/ROLES_PERMISSIONS.md#enforcement-points`

#### docs/solutions/I18N_WORKFLOW.md
...
- `docs/solutions/I18N_WORKFLOW.md#overview`
- `docs/solutions/I18N_WORKFLOW.md#missing-key-behavior`
...
| Consent-to-save profile updates | Not implemented (no profile persistence API/routes; `/profile` is placeholder `src/app/(seeker)/profile/page.tsx`) | `docs/VISION.md`; `docs/SECURITY_PRIVACY.md`; `docs/ui/UI_SURFACE_MAP.md` | None | PARTIAL (safe-by-absence) | Before adding profile persistence: implement explicit consent state + server-side enforcement; add tests around “no writes without consent.” |
...
| Input validation with Zod | `src/app/api/chat/route.ts` uses `ChatRequestSchema`; `src/app/api/search/route.ts` uses `SearchParamsSchema`; `src/app/api/feedback/route.ts` uses `FeedbackRequestSchema`; importer uses Zod schemas in `db/import/hsds-csv-importer.ts` | `docs/solutions/IMPORT_PIPELINE.md`; `docs/SECURITY_PRIVACY.md` | Implicitly tested via unit tests on chat/search/scoring modules; importer not tested | PASS | Add tests for API param schemas and importer row schemas if importer moves toward production usage. |
| “LLM summarization must not add facts” guardrails | Only gating exists: `src/services/chat/orchestrator.ts` `OrchestratorDeps.summarizeWithLLM?` + feature flag `FEATURE_FLAGS.LLM_SUMMARIZE`; no output verifier exists | `docs/VISION.md`; `docs/CHAT_ARCHITECTURE.md`; `docs/platform/INTEGRATIONS.md` | `src/services/chat/__tests__/intent-schema.test.ts` tests gate behavior, not factual containment | PARTIAL | Add a post-summary verifier (e.g., allowlist-only tokens/fields or structured summary format) + tests that reject out-of-record facts before enabling any LLM integration. |
...
- Likely files: `docs/SECURITY_PRIVACY.md`, `docs/platform/INTEGRATIONS.md`, `docs/ui/UI_SURFACE_MAP.md`, `docs/solutions/IMPORT_PIPELINE.md`, `docs/solutions/I18N_WORKFLOW.md`

1. **Wire retrieval to DB for search (end-to-end retrieval-first) or explicitly gate UI until wired**
...
2. **Fix `minConfidence` scale mismatch (0–1 vs 0–100)**
...
3. **Implement rate limiting consistently across APIs claimed in docs**
...
4. **Add LLM fact-containment guardrails BEFORE any LLM integration**
