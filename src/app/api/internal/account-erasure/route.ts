  ...
  try {
    ...
    while (claimed < 4 && Date.now() < deadlineMs) {
      ...
      results.push(await processAccountErasurePages(request, {
        // The current Vercel plan permits one cron delivery per day. An empty
        // 72-step manifest needs two verification passes, so let a light
        // request finish in one invocation while the shared deadline remains
        // the authoritative wall-clock bound.
        maxPages: 160,
        deadlineMs,
      }));
    }
