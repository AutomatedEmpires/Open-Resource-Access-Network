...
describe('outbound HTTP policy', () => {
  ...
  it('accepts a hostname only when every resolved address is public', async () => {
    ...
    await expect(
      validateOutboundHttpUrl('https://resources.example.org/feed', {
  ...
  it('fails closed for empty, malformed, and family-mismatched DNS answers', async () => {
    await expect(
      validateOutboundHttpUrl('https://resources.example.org', { resolver: resolver() }),
    ).rejects.toMatchObject({ code: 'dns_resolution_failed' });
    await expect(
      validateOutboundHttpUrl('https://resources.example.org', {
  ...
  it('aborts a pending DNS resolution instead of waiting past the request deadline', async () => {
