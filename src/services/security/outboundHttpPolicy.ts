...
import { isProhibitedMicrosoftEndpoint } from '@/services/runtime/providerPolicy';
...
/**
 * Synchronous preflight for outbound HTTP URLs. DNS resolution and
 * connection-time rebinding protection can be layered behind this single seam.
 */
export function parseOutboundHttpUrl(rawUrl: string): URL {
  ...
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new OutboundHttpPolicyError('blocked_scheme', 'Outbound URL must use HTTP or HTTPS');
  }
  ...
  if (isProhibitedMicrosoftEndpoint(url.href)) {
    throw new OutboundHttpPolicyError(
      'blocked_provider',
      'Outbound URL uses a prohibited provider endpoint',
    );
  }
...
async function resolveWithSignal(
  ...
  return new Promise((resolve, reject) => {
    const onAbort = () => {
      reject(new OutboundHttpPolicyError('aborted', 'Outbound URL validation was aborted', {
        cause: signal.reason,
      }));
    };
...
export function createValidatedLookup(resolver: OutboundDnsResolver = defaultResolver) {
...
const defaultTlsDispatcher = new Agent({
  connect: { lookup: createValidatedLookup(defaultResolver) },
});

const defaultInsecureTlsDispatcher = new Agent({
  connect: {
    lookup: createValidatedLookup(defaultResolver),
    rejectUnauthorized: false,
  },
});

/**
 * Shared dispatcher that repeats public-address validation during socket DNS
 * lookup, closing the preflight-to-connect rebinding window.
 */
