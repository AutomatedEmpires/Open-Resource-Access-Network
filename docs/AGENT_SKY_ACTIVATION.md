...
2. `docs/UI_UX_CONTRACT.md` — non-negotiable UI standards (MUST / SHOULD / MAY)
3. `docs/UI_UX_TOKENS.md` — spacing, typography, color semantic tokens
4. `docs/SECURITY_PRIVACY.md` — what must never happen (PII, GPS, hallucination)
5. `docs/OPERATING_MODEL.md` — how changes are validated before shipping
...
- Design tokens doc: `docs/UI_UX_TOKENS.md`
...
5. After build succeeds — record the ACTUAL bundle sizes in `docs/STATUS_OMEGA.md` (replacing the fabricated numbers).
...
- HOWEVER: mark this as a documented exception in `docs/UI_UX_TOKENS.md` with justification
...
3. Document the approved exceptions list in `docs/UI_UX_TOKENS.md` §8 "Approved Arbitrary Values".
...
**Define which option to take and implement it. Update `docs/I18N_WORKFLOW.md` with findings.**
