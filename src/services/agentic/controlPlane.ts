...
export async function buildAgentControlPlaneSnapshot(
  ...
  const integrations: ControlPlaneIntegrationStatus[] = [
    ...
    },
    {
      ...
      label: 'Azure Maps',
      category: 'mapping',
      state: integrationState(['AZURE_MAPS_KEY'], env),
      requiredEnv: ['AZURE_MAPS_KEY'],
      missingEnv: ['AZURE_MAPS_KEY'].filter((key) => !hasEnv(env, key)),
      powers: ['service map token brokering', 'geospatial search', 'resource discovery'],
    },
    {
      id: 'ai_core',
