...
- **Documentation changes must be data-driven.** Only update docs to match what the code
  ...
  "Planned" labels.
- **SSOT alignment**: when you change an admin flow, update `docs/ROLES_PERMISSIONS.md`,
  `docs/GOVERNANCE.md`, and `docs/UI_SURFACE_MAP.md` as appropriate.
...
- **Status output**: at the end of your session, write a complete structured status report to
  `docs/STATUS_APEX.md` using the format defined at the bottom of this file.
...
4. `docs/ROLES_PERMISSIONS.md` — the full role hierarchy and per-role permissions
5. `docs/GOVERNANCE.md` — how ORAN is governed; approval workflows; admin authorities
6. `docs/UI_UX_CONTRACT.md` — admin UIs must also follow this contract
7. `docs/UI_UX_TOKENS.md` — concrete design parameters (use `max-w-7xl` for dashboards)
8. `docs/UI_SURFACE_MAP.md` — confirm the admin surface map is accurate
9. `docs/AUDIT_REPORT.md` — existing audit findings including admin flows
10. `docs/DEPLOYMENT_AZURE.md` — Azure deployment documentation
11. `docs/PLATFORM_AZURE.md` — Azure platform architecture
...
the role requirements in `docs/ROLES_PERMISSIONS.md` and the governance workflows
in `docs/GOVERNANCE.md`. No admin action can be unguarded or unlogged.

### 3.1 Admin Role Inventory
For every admin surface, document in `docs/STATUS_APEX.md`:
- Route path
- Required role (per `docs/ROLES_PERMISSIONS.md`)
...
- Layouts must use `max-w-7xl` for dashboards per `docs/UI_UX_TOKENS.md`.
...
- `docs/GOVERNANCE.md` — verify approval workflow matches what you just built.
- `docs/ROLES_PERMISSIONS.md` — verify every role listed maps to implemented behavior.
- `docs/UI_SURFACE_MAP.md` — add all admin routes built or confirmed as existing.
- `docs/DEPLOYMENT_AZURE.md` — verify Azure deployment steps are current with `scripts/azure/`.
- `docs/PLATFORM_AZURE.md` — verify Azure service list is current.
- `docs/INTEGRATIONS.md` — verify any API used by admin portals (not owned by SIGMA,
  but may document consumption of SIGMA's routes).
- `docs/AGENTS_BUILD_PLAN.md` — update with current completion status.
...
- Cross-reference `docs/DEPLOYMENT_AZURE.md` to ensure it reflects the current scripts.
...
- [ ] `docs/GOVERNANCE.md` accurately reflects the built approval workflow.
- [ ] `docs/ROLES_PERMISSIONS.md` matches the implemented role set.
- [ ] `docs/UI_SURFACE_MAP.md` includes all admin routes.
- [ ] `docs/DEPLOYMENT_AZURE.md` is reconciled with current scripts.
