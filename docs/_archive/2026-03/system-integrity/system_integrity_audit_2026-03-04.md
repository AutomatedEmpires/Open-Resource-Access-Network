...
2. Transition history and SLA tables exist.
...
3. Index coverage exists for common queue/reporting patterns.
...
4. Legacy queue migration and compatibility view are present.
...
2. Nullable/auth mismatch for `submitted_by_user_id` on service submissions.
...
3. Domain types/constants drift from DB constraints.
...
2. Queue/review/admin routes read/write `submissions` and use workflow engine.
...
3. Permissions are enforced in migrated routes.
...
2. Community verify UI decision enum is legacy.
...
3. ORAN admin approvals UI sends wrong key and uses legacy statuses.
...
2. Transition history is recorded, but not all lock lifecycle events are transition-coupled.
...
2. Community verification confidence update uses new submission relation.
...
2. Publish endpoint marks candidate as published but does not write live `services` row in this route.
...
2. **High**: Community verify decision enum mismatch (`verified/rejected`) vs API (`approved/denied`) breaks decision submissions.
- `src/app/(community-admin)/verify/page.tsx:116`
- `src/app/api/community/queue/[id]/route.ts:28`

3. **High**: ORAN admin approvals POST key mismatch (`queueEntryId` vs `submissionId`) breaks approvals.
- `src/app/(oran-admin)/approvals/page.tsx:153`
- `src/app/api/admin/approvals/route.ts:38`

4. **High**: UI filters use legacy statuses (`verified/rejected/in_review/pending`) that do not match submission status enums.
...
5. **Medium**: Lock can remain after failed claim transition.
- `src/app/api/community/queue/route.ts:219`
- `src/app/api/community/queue/route.ts:236`

6. **Medium**: Host claim phone field has no persistence path.
- `src/app/(host)/claim/page.tsx:66`
- `src/app/(host)/claim/page.tsx:271`
- `src/app/api/host/claim/route.ts:23`

7. **Medium**: DB/app enum drift (`system` target type; `high` vs `elevated` scope risk level).
...
8. **Medium**: Ingestion review/publish operates on parallel status model, not unified submissions workflow.
- `src/app/api/admin/ingestion/candidates/route.ts:24`
- `src/app/api/admin/ingestion/candidates/[id]/route.ts:25`

9. **Test quality risk**: test run exits non-zero due unhandled error despite 148 files passing.
