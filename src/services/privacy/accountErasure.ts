...
export class AccountErasureUnavailableError extends Error {
  constructor() {
    super('Account erasure capability is not ready');
    this.name = 'AccountErasureUnavailableError';
  }
}

export function isAccountErasureUnavailable(
  error: unknown,
): error is AccountErasureUnavailableError {
  return error instanceof AccountErasureUnavailableError;
}
...
export async function queueAccountErasure(
  ...
  let rows: QueueRow[];
  try {
    rows = await executeQuery<QueueRow>(
      `SELECT request_id, request_status, lease_acquired
    ...
    if (
      ...
      && databaseError.message === 'account erasure capability is not ready'
    ) {
  ...
  const row = rows[0];
  if (!row?.request_id
      || typeof row.lease_acquired !== 'boolean'
...
async function releaseLease(requestId: string): Promise<void> {
  await executeQuery(
    `SELECT oran_internal.release_account_erasure_lease($1::uuid)`,
    [requestId],
  );
}
...
export async function processAccountErasure(
  ...
  try {
    ...
    const result: AccountErasureProcessingResult = {
    ...
    if (row.request_status === 'processing' && options.retainLease !== true) {
      await releaseLease(request.requestId);
    }
    return result;
  } catch {
...
export async function processAccountErasurePages(
  ...
  const maxPages = Math.max(1, Math.min(options.maxPages ?? 8, 160));
  ...
  do {
    result = await processAccountErasure(current, { retainLease: true });
    pagesProcessed += 1;
